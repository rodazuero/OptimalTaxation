function marginal_taxes(ctrlvec::Array{Float64},θvec::Array{Float64},solvec::Array{Float64},pa)
    ...
    for j=Nspan:-1:1
      ...
      τ_prime[j,2] = (λ*pa.α*e*(nn-pa.ς)^(pa.α-1)-ω)/ω; #τ_n_prime
...
function taxes_path(ctrlvec::Array{Float64},θvec::Array{Float64},solvec::Array{Float64},pa,θevec::Array{Float64},solvece::Array{Float64},ctrlvece::Array{Float64},margtax::Array{Float64},margtaxe::Array{Float64})
    ...
    for j = 1:Nspan
      ...
      nn = ctrlvec[j,2];
      ll = ctrlvec[j,3];
      tax_base[j,2] = ω*(nn-pa.ς); #Tn
      tax_base[j,3] = θ*ll*ω; #Tl
      #println("j = ", j, "tax_base Tn = ", tax_base[j,2], "tax_base Tl = ", tax_base[j,3])
    end
    ...
    for j = 1:Nspan
      ...
      ll = ctrlvec[j,3];
      Tn = Taxes_gb[j,2];
      tax_base[j,1] = e*nn^pa.α-ω*(nn-pa.ς)-Tn-zz; #Tc
    end
