function NotInfnew_find_controlse( θ, θe, sse, pa)
...
function NotInfrecover_controlse!(ctrlvec::Array{Float64}, θw::Float64 ,θvec::Array{Float64}, solvec::Array{Float64})
    (Nspan,~)=size(solvec)
    ...
    for j=Nspan:-1:1
      ...
      λe  = solvec[j,4];
      ωe  = solvec[j,6];
      sse = NotInfStateE(ue, μe, λe, ωe);

      (zze, nne)   = NotInfnew_find_controlse( θw, θe, sse, pa)
      ctrlvec[j,1] = zze;
      ctrlvec[j,2] = nne;
    end
    nothing
