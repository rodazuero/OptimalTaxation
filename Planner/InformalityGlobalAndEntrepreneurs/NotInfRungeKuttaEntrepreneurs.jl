function NotInfmy_runge_kuttae_reverse!(solution::Array{Float64},y_end,xspan,step,pa,θw; verbose = false)
    ...
    for i=1:Nspan
        ...
        #println("z1")
        NotInffind_statese!(z1, y, pa, θw, θe, ini);
        any(isnan,z1) && error("z1 is NaN ")
        #println("z2")
        NotInffind_statese!(z2, y+0.5*step*z1, pa, θw, x+0.5*step, ini );
        any(isnan,z2) && error("z2 is NaN ")
        #println("z3")
        NotInffind_statese!(z3, y+0.5*step*z2, pa, θw, x+0.5*step, ini );
        any(isnan,z3) && error("z3 is NaN ")
        #println("z4")
        NotInffind_statese!(z4, y+step*z3, pa, θw, x+step, ini);
