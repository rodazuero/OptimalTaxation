...
## Participacion e inactivos

#Proporcion de inactivos
ENAHO$inactivo<-NA
ENAHO$inactivo[ENAHO$condocup_ci=="Inactivo"]<-1
ENAHO$inactivo[ENAHO$condocup_ci=="Ocupado"|ENAHO$condocup_ci=="Desocupado"]<-0
sum(!is.na(ENAHO$inactivo))

H20<-ENAHO%>%
  group_by(perc_ingt)%>%
  summarise(ocupados=sum(emp_ci), desocupados=sum(desemp_ci), inactivos=sum(inactivo,na.rm = T))

H20$Percentil<-c(seq(1/nq,1,1/nq))
H20$Participacion<-H20$ocupados/(H20$ocupados+H20$desocupados)
H20$propinactivos<-H20$inactivos/(H20$ocupados+H20$desocupados+H20$inactivos)

Y20<-ggplot(data=H20,aes(x=Percentil,y=propinactivos))+
  geom_line()+
  labs(x=expression(theta~w),y="Inactivos/PET")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
dev.set()
png(file="Inactivos_PET.png",width=1600,height=850)
Y20
dev.off()

Y20.B<-ggplot(data=T20,aes(x=Percentil,y=Participacion))+
  geom_line()+
  labs(x=expression(theta~w),y="Participation in labor market")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
dev.set()
png(file="Participation in labor market.png",width=1600,height=850)
Y20.B
dev.off()
...
#Version 1A: Trabajadores/Horas actividad principal
H1.A<-ENAHOT%>%
  group_by(percentiling)%>%
...
H1.A$mediana<-H1.A$horas/H1.A$horas[5]


GP1.A<-ggplot(data=H1.A,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
dev.set()
png(file="GP1.A.png",width=1600,height=850)
GP1.A
dev.off()

#Version 1B: Trabajadores/Horas actividad principal (Mayor a 0)

H1.B<-ENAHOT[which(ENAHOT$horaspri_ci>0),]%>%
  group_by(percentiling)%>%
  summarise(horas=mean(horaspri_ci,na.rm = T))


H1.B$Percentil<-c(seq(1/nq,1,1/nq))

H1.B$mediana<-H1.B$horas/H1.B$horas[5]


GP1.B<-ggplot(data=H1.B,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
dev.set()
png(file="GP1.B.png",width=1600,height=850)
GP1.B
dev.off()

#Version 2A: Trabajadores/Horas totales 

H2.A<-ENAHOT%>%
  group_by(percentiling)%>%
  summarise(horas=mean(horastot_ci,na.rm = T))

sum(is.na(ENAHOT$horastot_ci))

H2.A$Percentil<-c(seq(1/nq,1,1/nq))

H2.A$mediana<-H2.A$horas/H2.A$horas[5]


GP2.A<-ggplot(data=H2.A,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
dev.set()
png(file="GP2.A.png",width=1600,height=850)
GP2.A
dev.off()

#Version 2B: Trabajadores/Horas totales (Mayor a 0)

H2.B<-ENAHOT[which(ENAHOT$horastot_ci>0),]%>%
  group_by(percentiling)%>%
  summarise(horas=mean(horastot_ci,na.rm = T))


H2.B$Percentil<-c(seq(1/nq,1,1/nq))

H2.B$mediana<-H2.B$horas/H2.B$horas[5]


GP2.B<-ggplot(data=H2.B,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
dev.set()
png(file="GP2.B.png",width=1600,height=850)
GP1.B
dev.off()
...
#Version 3 (PEA (incluye a quienes no trabajan) // Nivel de ingresos del hogar)
ENAHOPEA<-ENAHO[which(ENAHO$condocup_ci=="Ocupado"|ENAHO$condocup_ci=="Desocupado"),]
ENAHOPEA$percentil_hogar<-ntile(ENAHOPEA$ing_ch_pc,nq)

H3<-ENAHOPEA[!is.na(ENAHOPEA$ing_ch_pc),]%>%
  group_by(percentil_hogar)%>%
  summarise(horas=mean(horas_trab,na.rm = T))

H3$Percentil<-c(seq(1/nq,1,1/nq))
H3$mediana<-H3$horas/H3$horas[5]

GP3<-ggplot(data=H3,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
#Version 4 PEA (sin independientes) // Nivel de ingresos del hogar)
...
sum(!(ENAHO$categopri_ci==1))

H4<-ENAHOPEA4[!is.na(ENAHOPEA4$ing_ch_pc),]%>%
  group_by(percentil_hogar)%>%
  summarise(horas=mean(horas_trab,na.rm = T))

H4$Percentil<-c(seq(1/nq,1,1/nq))
H4$mediana<-H4$horas/H4$horas[5]

GP4<-ggplot(data=H4,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
dev.set()
png(file="HoursWorkedPEA(without entrepreneurs).png",width=1600,height=850)
GP4
dev.off()
...
H5<-ENAHOPET[!is.na(ENAHOPET$ing_ch_pc),]%>%
  group_by(percentil_hogar)%>%
  summarise(horas=mean(horas_trab,na.rm = T))

H5$Percentil<-c(seq(1/nq,1,1/nq))
H5$mediana<-H5$horas/H5$horas[5]

GP5<-ggplot(data=H5,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
#Version 6 (Version 2 + Eliminando las 74 observaciones extrañas)
...
H6$Percentil<-c(seq(1/nq,1,1/nq))

H6$mediana<-H6$horas/H6$horas[5]


GP6<-ggplot(data=H6,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
dev.set()
png(file="GP6.png",width=1600,height=850)
GP6
dev.off()

#Version 7 (Version 4 + Eliminando las 74 observaciones extrañas)
...
H7$Percentil<-c(seq(1/nq,1,1/nq))

H7$mediana<-H7$horas/H7$horas[5]


GP7<-ggplot(data=H7,aes(x=Percentil,y=mediana))+
  geom_line()+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"))
...
dev.set()
png(file="GP7.png",width=1600,height=850)
GP7
dev.off()
...
GRAPH<-as.data.frame(cbind(H1.A[,3:4],H1.B[,4],H2.A[,4],H2.B[,4],H3[,4],H4[,4],H5[,4],H6[,4],H7[,4]))
names(GRAPH)<-c("Percentil","Version 1A","Version 1B","Version 2A","Version 2B","Version 3","Version 4","Version 5","Version 6","Version 7")


TOTAL<-ggplot(data=GRAPH,aes(Percentil))+
  geom_line(aes(y=`Version 1A`,colour="V1: Trabajadores (Horas actividad principal)"))+
  geom_line(aes(y=`Version 2A`,colour="V2: Trabajadores (Horas totales)"))+
  geom_line(aes(y=`Version 3`,colour="V3: PEA"))+
  geom_line(aes(y=`Version 4`,colour="V4: PEA (excluye emprendedores)"))+
  geom_line(aes(y=`Version 5`,colour="V5: PET (18-65 a?os)"))+
  geom_line(aes(y=`Version 6`,colour="V6: V2 sin observaciones extra?as",linetype="dotted"))+
  geom_line(aes(y=`Version 7`,colour="V7: V4 sin observaciones extra?as",linetype="dotted"))+
  labs(x=expression(theta~w),y="Number of hours worked per week")+
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24,face="bold"),
...
dev.set()
png(file="TODAS.png",width=1600,height=850)
TOTAL
dev.off()
