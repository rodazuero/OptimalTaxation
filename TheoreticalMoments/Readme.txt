...
git clone -b C_in_Optimal --single-branch https://github.com/rodazuero/OptimalTaxation.git

git clone -b Alpha_in_Optimal --single-branch https://github.com/rodazuero/OptimalTaxation.git

git clone -b master --single-branch https://github.com/rodazuero/OptimalTaxation.git


running until 411 parameters. 
paro en here 4. 
...
#Problem with the now new sobol of dimension 15,000: Informality proportion demanded. 
It is very low, should be necessary to identify what elements make informality high. 

I fucked it with generating solos in C between 0 and 200. It should be much, much smaller. 

Change the way loading directly the parameters into a couple of functions:
StandardizedExcessDemands. 


Production and income is at levels, fitting them hard at the same weight with the other variables. Tried simply to put them in relative terms and see what happens. 


#MainOptimizer included some headers (rcpp.h. r.h. etc. and needed to include two different directories on the head search (dir3 y dir4) to be able to use the packages. One of them is /usr/lib64/R/include because i got the error term could not locate r.h. 

simply put the include armadillo in the situation solves everything for the mainOptimier.cpp


#Optimizer: problem with NaN generated in total labor supply. For some reason, we obtain nan in the labor supply generated by individuals. Why? need to check this. The problem is that the labor supply is relative to the median. If median does not provide labor supply, it is nothing. Need to fix this. 

Optimizer 2. TotalLaborSupply doing fine. Problema con contribución a pago de impuestos. Producción e Informal labor supply, lo demás está funcionando bonito. 


Optimizer 3. Failure. Bad parameter initialization. 

Sobol 3: MISTAKE: for loop was just until 5,000. NOBS was set at 52000

Sobol 4: Fixing that mistake. At 16789 it reaches an error malloc type. I will do shorter solos in the future to see what is going on. 

Optim4: Using as input the best fit of sobol 4. 
aalpha	ggamma	ddelta	bbeta	ssigma	kkappa	psi	chi	rho	mmu1	mmu2	ssigma1	ssigma2	rho12	c	NA	Indic
0.58	3.83	142.52	541.68	8.91	2.58	8.02	85.53	263.35	0.84	0.15	4.23	2.12	0.94	22.6	NA	1


New version of mainOptimizer.cpp: Contribute´øn de pago a salarios hay veces que da NaN. It is necessary to change that. Done in this current version for the subsequent optim5. 