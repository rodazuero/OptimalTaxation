...
library(mvtnorm)
Rcpp::sourceCpp('~/Dropbox/OptmalTaxationShared/Data/CodeV14/Equilibrium2/Equilibrium2/main.cpp')
...
ExcessDemandFunctions<-function(Wages,Params,M,InitLWorkers,InitProf){
  ...
  for(i in 1:M){
    ...
    logtthetavec<-rmvnorm(1, mean = mu, Sigma)
    tthetaw<-exp(logtthetavec[1,1])
    tthetae<-exp(logtthetavec[1,2])
    #tthetaw<-runif(1)+1
    #tthetae<-runif(1)+1
    #tthetaw<-1.2
    #tthetae<-5.5
